...
struct ToDoUseCaseTests {
    ...
    @Test("Create with empty title throws emptyTitle")
    func createWithEmptyTitleThrows() async {
        let sut = ToDoUseCase(repository: MockToDoRepository())
    ...
    @Test("Create with whitespace-only title throws emptyTitle")
    func createWithWhitespaceTitleThrows() async {
        let sut = ToDoUseCase(repository: MockToDoRepository())
    ...
    @Test("Update with empty title throws emptyTitle")
    func updateWithEmptyTitleThrows() async throws {
        let repository = MockToDoRepository()
        let sut = ToDoUseCase(repository: repository)
    ...
    func updateNonExistentThrows() async {
        let sut = ToDoUseCase(repository: MockToDoRepository())
        let nonExistent = ToDo.fixture()

        await #expect(throws: ToDoRepositoryError.notFound) {
            try await sut.update(nonExistent)
        }
    }
